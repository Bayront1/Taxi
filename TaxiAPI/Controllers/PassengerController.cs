...
namespace TaxiAPI.Controllers
{
    ...
    public class PassengerController : ControllerBase
    {
        ...
        public ActionResult<IEnumerable<User>> Ride(int passengerID)
        {
            try
            {
                var ride = _context.Rides.SingleOrDefault(pr => pr.PassengerId == passengerID);

                if (ride == null)
                {
                    return NotFound("Поездок для указанного пассажира не найдено.");
                }

                var driver = _context.Users.FirstOrDefault(pr => pr.UserId == ride.DriverId);
        ...
        [HttpDelete("DeletePassengerRequest/{id}")]
        public async Task<IActionResult> DeletePassengerRequest(int? id)
        {
            if (_context.Passengerrequests == null)
            {
                return NotFound();
            }

            var user = await _context.Passengerrequests.FindAsync(id);
            if (user == null)
            {
                return NotFound();
            }

            _context.Passengerrequests.Remove(user);
            await _context.SaveChangesAsync();
        ...
        public async Task<IActionResult> DeleteRide(int? id)
        {
            ...
            var user = await _context.Rides.FindAsync(id);
            if (user == null)
            {
                return NotFound();
            }

            _context.Rides.Remove(user);
            await _context.SaveChangesAsync();
